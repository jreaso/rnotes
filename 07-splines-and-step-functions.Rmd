...
For smoothing splines we use `smoothing.splines()` instead of `lm()`. Use the `df` parameter to specify effective degrees of freedom or the `cv = TRUE` parameter to use cross validation instead (to choose a $\lambda$). Can also directly specify `lambda` or `spar` (a smoothing parameter).
